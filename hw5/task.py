...
from typing import Any, overload, Protocol, Callable, TypeVar
...
class MediaFile:
    ...
    def __init__(self, data: Data | None = None, info: Info | None = None, meta: Meta | None = None):
        ...
        Данных на момент создание экземпляра еще может не быть, поэтому они Optional.
        Info и Meta не всегда требуются, чтоб не блокировали создание экземпляров, также делаем их
        Optional.
        """
    ...
    def read(self, size: str = 'all'):
        ...
        Может считать все данные, либо 1 строку, либо указанное кол-во байт.
        Также можно реализовать считывание с фильтром по каким-либо именованным параметрам.
        Может вернуть считанные данные, либо сохранить во внутреннем контейнере.
        """
